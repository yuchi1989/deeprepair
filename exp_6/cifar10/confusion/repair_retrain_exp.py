...
def main():
    ...
    if args.dataset.startswith('cifar'):
        ...
        if args.dataset == 'cifar100':
            ...
            target_train_loader = torch.utils.data.DataLoader(target_train_dataset, batch_size=args.batch_size//5, shuffle=True, 
                                        num_workers=args.workers, pin_memory=True)
            target_val_loader = torch.utils.data.DataLoader(target_test_dataset, batch_size=args.batch_size//5, shuffle=True, 
                                        num_workers=args.workers, pin_memory=True)
...
def train(train_loader, target_train_loader, model, criterion, optimizer, epoch):
    ...
    for i, ((input, target), (target_input, target_target)) in enumerate(zip(train_loader, cycle(target_train_loader))):
...
def validate(val_loader, target_val_loader, model, criterion, epoch):
    ...
    for i, ((input, target), (target_input, target_target)) in enumerate(zip(val_loader, cycle(target_val_loader))):
        target = target.cuda()
        target_copy = target_target.cpu().numpy()
        output = model(input)
        target_output = model(target_input)
        ...
        if len(id3) == 0 or len(id5) == 0:
            ...
            p_dist = torch.dist(torch.mean(
                m(target_output)[id3], 0), torch.mean(m(target_output)[id5], 0), 2)
        loss2 = loss.mean() - args.lam*p_dist
        # measure accuracy and record loss
        err1, err5 = accuracy(output.data, target, topk=(1, 5))
