...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=256) # batch size should be smaller if use text
    ...
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        print(str((args.second, args.third)) + " pairwise bias: " + 
            str(compute_pairwise_bias(confusion_matrix, args.second, args.third, id2object)))
...
def compute_pairwise_bias(confusion_matrix, second, third, id2object):
    object_list = []
    for i in range(80):
...
def train(args, epoch, model, criterion, train_loader, optimizer, train_F, score_F, train_data):
    ...
    for batch_idx, (images, objects, image_ids) in enumerate(t):
        ...
        for j in range(len(labels)):
            for k in labels[j]:
                if k == args.second or k == args.third:
                    continue
                if k in label_count:
                    label_count[k] += 1
                else:
                    label_count[k] = 1
        first = random.choice(Counter(label_count).most_common(20))[0]

        #first = max(label_count.iteritems(), key=operator.itemgetter(1))[0]
        for j in range(len(labels)):
            if first in (labels[j]):# and "bus" not in (labels[j]):
                firstid.append(j)
