        ...
        for j in range(10):
            for k in range(10):
                if i == j or j == k or i ==k:
                    continue
                triplet_bias[(i, j, k)] = abs(confusion_matrix[(i, j)] - confusion_matrix[(i, k)])
    keys = sorted(triplet_bias, key=triplet_bias.get, reverse=True)[:n]
    for i in range(n):
        print("")
        print(str(keys[i]) + " triplet: " + str(triplet_bias[keys[i]]))
        print(str((keys[i][0],keys[i][1])) + ": " + str(confusion_matrix[(keys[i][0], keys[i][1])]))
        print(str((keys[i][0],keys[i][2])) + ": " + str(confusion_matrix[(keys[i][0], keys[i][2])]))
...
if __name__ == '__main__':
    ...
    top_bias("../../exp_2/log/cifar10_resnet_2_4_epoch_confusion.npy", 6)