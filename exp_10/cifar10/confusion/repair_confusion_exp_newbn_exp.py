    ...
    for attr_str in dir(module):
        ...
        if type(target_attr) == torch.nn.BatchNorm2d:
            ...
            if glob_bn_count >= glob_bn_total - 2:  # unfreeze last 3
                print('replaced: bn')
                new_bn = dnnrepair_BatchNorm2d(target_attr.num_features, target_attr.weight, target_attr.bias, target_attr.running_mean, target_attr.running_var, 10/19, target_attr.eps, 0.19, target_attr.affine, track_running_stats=True)
...
def main():
    ...
    print('Repair Confusion NewBN Exp Best accuracy (top-1 and 5 error):', best_err1, best_err5, file=open("output.txt", "a"))
